diff --git a/backend/internal/service/tasks/service.go b/backend/internal/service/tasks/service.go
index b69da79..29d96cf 100644
--- a/backend/internal/service/tasks/service.go
+++ b/backend/internal/service/tasks/service.go
@@ -707,7 +707,18 @@ func (s *Service) Heartbeat(ctx context.Context, actor identity.Identity, projec
...
+	result, err := tx.ExecContext(ctx, `
 		...
 		SET last_heartbeat_at = NOW(),
 			updated_at = NOW()
@@ -731,6 +742,19 @@ func (s *Service) Heartbeat(ctx context.Context, actor identity.Identity, projec
...
+
 	if checkpoint := strings.TrimSpace(input.Checkpoint); checkpoint != "" && s.memoryWriter != nil {
 		_, writeErr := s.memoryWriter.Write(ctx, projectID, openviking.WriteInput{
 			Target:  "note",
@@ -1707,11 +1731,18 @@ func (s *Service) BlockTimedOutRunningTasks(ctx context.Context, heartbeatTimeou
 	staleBefore := time.Now().UTC().Add(-heartbeatTimeout)
 
 	rows, err := s.db.QueryContext(ctx, `
-		SELECT id, project_id, session_id, COALESCE(active_run_id, '')
-		FROM tasks
-		WHERE status = 'running'
-		  AND COALESCE(last_heartbeat_at, updated_at) < $1
-		ORDER BY COALESCE(last_heartbeat_at, updated_at) ASC
...
+			  ) ASC
 	...
 	`, staleBefore, limit)
 	if err != nil {
diff --git a/backend/internal/service/tasks/service_test.go b/backend/internal/service/tasks/service_test.go
index 5aff573..8548b6d 100644
--- a/backend/internal/service/tasks/service_test.go
+++ b/backend/internal/service/tasks/service_test.go
@@ -373,6 +373,114 @@ func TestResumeRejectsExistingRunOwnedByAnotherAgent(t *testing.T) {
 	}
 }
...
+
 func TestValidateCreateInputIgnoresDelegateType(t *testing.T) {
 	details := validateCreateInput("prj_1", CreateTaskInput{
 		Title:       "ok title",
diff --git a/fronted/components.json b/fronted/components.json
new file mode 100644
index 0000000..5c23ec4
--- /dev/null
+++ b/fronted/components.json
@@ -0,0 +1,25 @@
+{
...
+  "registries": {}
+}
diff --git a/fronted/e2e/settings-openviking.spec.ts b/fronted/e2e/settings-openviking.spec.ts
new file mode 100644
index 0000000..0b06c70
--- /dev/null
+++ b/fronted/e2e/settings-openviking.spec.ts
@@ -0,0 +1,106 @@
+import { test, expect } from '@playwright/test'
+
+test.describe('OpenViking Settings', () => {
+  test('should load settings and save successfully', async ({ page }) => {
+    // Mock the project detail API which is also loaded on the settings page
...
+    await page.route('**/api/projects/prj_mock/openviking/settings', async (route) => {
+      if (route.request().method() === 'GET') {
+        await route.fulfill({
+          status: 200,
+          json: {
...
+        // API key should only be sent if dirty, in this test we don't change it initially
+        if (payload.namespace === 'new-namespace') {
...
+
+    // Mock the status API
+    await page.route('**/api/projects/prj_mock/openviking/status', async (route) => {
+      if (route.request().method() === 'GET') {
+        await route.fulfill({
+          status: 200,
+          json: {
...
+    // Assert toast
+    await expect(
+      page.getByText('OpenViking 设置已保存').or(page.getByText('OpenViking settings saved')).first()
+    ).toBeVisible()
+  })
+})
diff --git a/fronted/package.json b/fronted/package.json
index d15ffb8..f14e899 100644
--- a/fronted/package.json
+++ b/fronted/package.json
@@ -21,6 +21,7 @@
   "dependencies": {
     "@dnd-kit/core": "^6.3.1",
...
diff --git a/fronted/pnpm-lock.yaml b/fronted/pnpm-lock.yaml
index ab0355b..536d918 100644
--- a/fronted/pnpm-lock.yaml
+++ b/fronted/pnpm-lock.yaml
@@ -14,6 +14,9 @@ importers:
       '@dnd-kit/utilities':
         specifier: ^3.2.2
...
+      shadcn:
+        specifier: ^4.7.0
+        version: 4.7.0(@types/node@24.12.2)(typescript@6.0.3)
       tailwind-merge:
         specifier: ^3.5.0
         version: 3.5.0
...
@@ -164,7 +179,7 @@ importers:
       ...
       vitest:
         specifier: ^4.1.5
-        version: 4.1.5(@types/node@24.12.2)(@vitest/coverage-v8@4.1.5)(jsdom@29.1.1)(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))
+        version: 4.1.5(@types/node@24.12.2)(@vitest/coverage-v8@4.1.5)(jsdom@29.1.1(@noble/hashes@1.8.0))(msw@2.14.3(@types/node@24.12.2)(typescript@6.0.3))(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))
 
 packages:
 
@@ -202,14 +217,28 @@ packages:
...
@@ -3339,7 +5367,7 @@ snapshots:
       obug: 2.1.1
       std-env: 4.1.0
       tinyrainbow: 3.1.0
-      vitest: 4.1.5(@types/node@24.12.2)(@vitest/coverage-v8@4.1.5)(jsdom@29.1.1)(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))
+      vitest: 4.1.5(@types/node@24.12.2)(@vitest/coverage-v8@4.1.5)(jsdom@29.1.1(@noble/hashes@1.8.0))(msw@2.14.3(@types/node@24.12.2)(typescript@6.0.3))(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))
 
   '@vitest/expect@4.1.5':
     dependencies:
@@ -3350,12 +5378,13 @@ snapshots:
       chai: 6.2.2
       tinyrainbow: 3.1.0
 
-  '@vitest/mocker@4.1.5(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))':
+  '@vitest/mocker@4.1.5(msw@2.14.3(@types/node@24.12.2)(typescript@6.0.3))(vite@8.0.10(@types/node@24.12.2)(jiti@2.6.1))':
     dependencies:
       '@vitest/spy': 4.1.5
       estree-walker: 3.0.3
...
+  setprototypeof@1.2.0: {}
+
+  shadcn@4.7.0(@types/node@24.12.2)(typescript@6.0.3):
+    dependencies:
+      '@babel/core': 7.29.0
+      '@babel/parser': 7.29.2
+      '@babel/plugin-transform-typescript': 7.28.6(@babel/core@7.29.0)
...
diff --git a/fronted/pnpm-workspace.yaml b/fronted/pnpm-workspace.yaml
new file mode 100644
index 0000000..8181285
--- /dev/null
+++ b/fronted/pnpm-workspace.yaml
@@ -0,0 +1,2 @@
+allowBuilds:
+  msw: set this to true or false
diff --git a/fronted/src/api/client.ts b/fronted/src/api/client.ts
index bfe444f..816ef5d 100644
--- a/fronted/src/api/client.ts
+++ b/fronted/src/api/client.ts
...
diff --git a/fronted/src/api/openviking-settings.ts b/fronted/src/api/openviking-settings.ts
new file mode 100644
index 0000000..32cf5bd
--- /dev/null
+++ b/fronted/src/api/openviking-settings.ts
@@ -0,0 +1,72 @@
...
+}
+
+export const openvikingKeys = {
+  all: ['openviking'] as const,
+  settings: (projectId: string) => [...openvikingKeys.all, 'settings', projectId] as const,
+  status: (projectId: string) => [...openvikingKeys.all, 'status', projectId] as const,
+}
+
+export function useOpenVikingSettingsQuery(projectId: string | undefined) {
+  return useQuery({
+    queryKey: projectId
+      ? openvikingKeys.settings(projectId)
+      : ['openviking', 'settings', 'unknown'],
+    queryFn: () => request<OpenVikingSettings>(`/api/projects/${projectId}/openviking/settings`),
+    enabled: Boolean(projectId),
+  })
+}
+
+export function useUpdateOpenVikingSettingsMutation(projectId: string | undefined) {
+  const queryClient = useQueryClient()
+  return useMutation({
+    mutationFn: (input: UpdateOpenVikingSettingsInput) =>
+      request(`/api/projects/${projectId}/openviking/settings`, {
+        method: 'PUT',
+        body: input,
+      }),
+    onSuccess: () => {
+      if (!projectId) return
+      queryClient.invalidateQueries({ queryKey: openvikingKeys.settings(projectId) })
+      queryClient.invalidateQueries({ queryKey: openvikingKeys.status(projectId) })
+    },
+  })
+}
+
+export function useOpenVikingStatusQuery(
+  projectId: string | undefined,
+  options?: { refetchInterval?: number },
+) {
+  return useQuery({
+    queryKey: projectId ? openvikingKeys.status(projectId) : ['openviking', 'status', 'unknown'],
+    queryFn: () => request<OpenVikingStatus>(`/api/projects/${projectId}/openviking/status`),
+    enabled: Boolean(projectId),
+    refetchInterval: options?.refetchInterval,
+  })
+}
diff --git a/fronted/src/components/layout/sidebar.tsx b/fronted/src/components/layout/sidebar.tsx
index 15acb2d..39700a2 100644
--- a/fronted/src/components/layout/sidebar.tsx
+++ b/fronted/src/components/layout/sidebar.tsx
@@ -51,11 +51,12 @@ const NAV_ITEMS: ReadonlyArray<NavItem> = [
...
@@ -82,10 +83,7 @@ export function Sidebar({ onCreateProject }: SidebarProps) {
       </div>
...
@@ -199,9 +197,7 @@ export function Sidebar({ onCreateProject }: SidebarProps) {
         </section>
 
         <nav className="space-y-1">
-          {!activeProject ? (
-            <AgentsNavGroup defaultOpen pathname={location.pathname} />
-          ) : null}
+          {!activeProject ? <AgentsNavGroup defaultOpen pathname={location.pathname} /> : null}
 
           {activeProject
             ? NAV_ITEMS.map((item) => (
diff --git a/fronted/src/components/ui/switch.tsx b/fronted/src/components/ui/switch.tsx
new file mode 100644
index 0000000..e27a77c
--- /dev/null
+++ b/fronted/src/components/ui/switch.tsx
@@ -0,0 +1,26 @@
...
+
+export { Switch }
diff --git a/fronted/src/i18n/locales/en.ts b/fronted/src/i18n/locales/en.ts
index 1db906a..93401ee 100644
--- a/fronted/src/i18n/locales/en.ts
+++ b/fronted/src/i18n/locales/en.ts
@@ -201,6 +201,23 @@ const en: typeof zhCN = {
...
diff --git a/fronted/src/i18n/locales/zh-CN.ts b/fronted/src/i18n/locales/zh-CN.ts
index 7abe7aa..6d8c3ea 100644
--- a/fronted/src/i18n/locales/zh-CN.ts
+++ b/fronted/src/i18n/locales/zh-CN.ts
@@ -206,6 +206,23 @@ const zhCN = {
...
+  },
   overview: {
     metrics: '关键指标',
     metricTotal: '总任务',
diff --git a/fronted/src/routes/agents.route.tsx b/fronted/src/routes/agents.route.tsx
index bb5fc09..1ae9857 100644
--- a/fronted/src/routes/agents.route.tsx
+++ b/fronted/src/routes/agents.route.tsx
@@ -29,8 +29,7 @@ export function AgentsRoute({ filterType }: AgentsRouteProps = {}) {
   const [revokingTokenId, setRevokingTokenId] = React.useState<string | undefined>()
   const [issueOpen, setIssueOpen] = React.useState(false)
 
-  const selectedAgent =
-    items.find((agent) => agent.agentId === selectedAgentId) ?? items[0]
...
@@ -277,7 +276,8 @@ function AgentDetail({ agent, onRevoke, onIssue }: AgentDetailProps) {
                         <div className="space-y-0.5">
                           <UlidChip id={token.tokenId} />
                           <p className="text-[11px] text-slate-500">
-                            过期 {token.expiresAt ? formatRelativeTime(token.expiresAt) : '永不过期'}
+                            过期{' '}
+                            {token.expiresAt ? formatRelativeTime(token.expiresAt) : '永不过期'}
                             {revoked ? ` · 撤销于 ${formatRelativeTime(token.revokedAt!)}` : ''}
                           </p>
                           <p className="font-mono text-[10px] text-slate-400">
diff --git a/fronted/src/routes/settings.route.tsx b/fronted/src/routes/settings.route.tsx
index d3e5c2e..8154994 100644
--- a/fronted/src/routes/settings.route.tsx
+++ b/fronted/src/routes/settings.route.tsx
@@ -35,6 +35,12 @@ import {
   useProjectQuery,
   useUpdateProjectMutation,
 } from '@/api/projects'
+import {
+  useOpenVikingSettingsQuery,
+  useUpdateOpenVikingSettingsMutation,
+  useOpenVikingStatusQuery,
+} from '@/api/openviking-settings'
...
@@ -218,91 +224,99 @@ export function SettingsRoute() {
 
   return (
     <div className="h-full overflow-y-auto bg-[hsl(var(--muted)/0.4)]">
-      <form className="mx-auto grid max-w-4xl gap-4 p-6" onSubmit={handleSave}>
-        <Card>
...
-                <Input id="settings-goal" {...form.register('goal')} />
-                {form.formState.errors.goal ? (
-                  <p className="text-[11px] text-rose-600">{form.formState.errors.goal.message}</p>
-                ) : null}
...
-          </Button>
-        </div>
+        <OpenVikingSettingsCard projectId={projectId} />
 
         <Card>
           <CardContent className="space-y-4 p-5">
@@ -360,7 +374,7 @@ export function SettingsRoute() {
             </div>
           </CardContent>
         </Card>
-      </form>
+      </div>
       ...
       <Dialog open={completeOpen} onOpenChange={setCompleteOpen}>
         <DialogContent>
@@ -448,3 +462,191 @@ function PolicyField({ control, field, errorMessage, onChange }: PolicyFieldProp
     </div>
   )
 }
+
...
+
+function OpenVikingSettingsCard({ projectId }: { projectId: string }) {
+  const { t } = useTranslation()
+  const { toast } = useToast()
+
+  const settingsQuery = useOpenVikingSettingsQuery(projectId)
+  const statusQuery = useOpenVikingStatusQuery(projectId, { refetchInterval: 30000 })
+  const updateMutation = useUpdateOpenVikingSettingsMutation(projectId)
+
...
+          apiKey: '',
+          namespace: settingsQuery.data.namespace,
+          workspaceId: settingsQuery.data.workspaceId,
...
+            <div className="grid gap-3 md:grid-cols-2">
+              <div className="space-y-1.5">
+                <Label htmlFor="ov-server-url">{t('settings.openviking.serverUrl')}</Label>
+                <Input id="ov-server-url" {...form.register('serverUrl')} />
...
+              </div>
+              <div className="space-y-1.5">
+                <Label htmlFor="ov-api-key">{t('settings.openviking.apiKey')}</Label>
+                <Input
+                  id="ov-api-key"
...
+              </div>
+              <div className="space-y-1.5">
+                <Label htmlFor="ov-namespace">{t('settings.openviking.namespace')}</Label>
+                <Input id="ov-namespace" {...form.register('namespace')} />
...
+              </div>
+              <div className="space-y-1.5">
+                <Label htmlFor="ov-workspace-id">{t('settings.openviking.workspaceId')}</Label>
+                <Input id="ov-workspace-id" {...form.register('workspaceId')} />
...
+                <Switch
+                  checked={form.watch('enableMemoryWrite')}
+                  onCheckedChange={(val) =>
